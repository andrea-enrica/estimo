import {Button, Card, Flex, Progress, Result, Spin, Tooltip, Tour, Typography} from "antd";
      ...
      {authUserRole === UserRole.MANAGER && authUserId === managerId && (
        <>
          <div className="manager-actions-container">
            ...
            >
              <Tooltip title={storyList.length == 0 && "Please add a story to start the session"}>
              <Button
                ref={refStartSession}
                className="manager-button"
                onClick={onStartSession}
                loading={!isConnected}
                disabled={isSessionActive || storyList.length == 0}
